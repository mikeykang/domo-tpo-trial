(function () {
  ...
  E.push('<div class="panel"><div class="ph"><h3>진단 9 · 한국은 빈도로 쓰고, 일본은 의식으로 쓴다' +
    ib('패드 리포트의 「기울기」 자리. 미스트 표본도 최근에 쏠려 있어 기울기 대신 행동 동사 비교를 넣었다. 코퍼스 전량 기준.') +
  ...
  E.push('<div class="foot">' +
    ...
    '<b>한계</b> 한국 리뷰는 전량이 아니라 <b>비로그인 층화 표본</b>이다 (' + n(D.corpus.krReviews) + ' / ' + n(D.corpus.krSiteTotal) + '건 = ' + (D.corpus.krReviews / D.corpus.krSiteTotal * 100).toFixed(1) + '%; 정렬 5축 × 리뷰유형 4축, 조합당 500건 상한). 대형 리스팅(달바 · 아벤느)일수록 표본 비율이 낮고 최근으로 쏠려 있어 <b>시간 기울기는 산출하지 않았다</b>. 일본 큐텐은 전량이지만 리뷰의 76%가 달바 한 제품이다 — 시장 합계가 아니라 제품별 분해로 읽어야 하는 이유고, 격자는 제품 동일가중으로 이를 보정했다. MIGUHARA는 후기 22건(랭킹은 최근 판매속도 가중이라 신제품이 위로 온다)이라 비율 계산에서 사실상 빠진다. 일본 상세페이지와 일본 광고는 수집하지 않았다(한국은 둘 다). 가격 · 원가 · 실제 전환은 이 데이터에 없다. 재구매 차이는 관찰 상관이지 실험이 아니다.<br>' +
